...
var d3Graphs = {
    ...
    setHistogramData: function () {
        ...
        var marketCap = selectedCountry.marketCap
        ...
        for (var i = 0; i < numHistory; i++) {
            var importPrev = marketCap[i % 3];
            // var importCur = marketCap[1];
            // var importDiff = (importCur - importPrev) / importPrev * 100;
            cloudArray.push({ x: i, y: i * marketCap[0] });
    ...
    drawBarGraph: function () {
        ...
        let aws = selectedCountry.marketCap[0]
        let gcp = selectedCountry.marketCap[1]
        let azure = selectedCountry.marketCap[2]
        // exportTotal = selectedCountry.marketCap[1]
        console.log('selectedcountry marketcap:', selectedCountry.marketCap)
        ...
        for (var type in reverseWeaponLookup) {
            ...
            importArray.push({ "type": type, "amount": aws });
            exportArray.push({ "type": type, "amount": gcp });
        }
        ...
        //     var value = d3Graphs.barGraphHeight - d3Graphs.barGraphBottomPadding - d3Graphs.cumImportY - yScale(10);
        //     d3Graphs.cumImportY += yScale(value);
        //     // return d3Graphs.cumImportY;
        //     return yScale(aws)
        // })
        // .attr('height', function (d) { return yScale(200); })
        // .attr('fill', 'red');
        ...
        // var cloudRects = this.barGraphSVG.selectAll('rect.cloud').data(exportArray);
        // cloudRects.enter().append('rect').attr('class', function (d) {
        //     return 'cloud ' + d.type;
        // }).attr('x', midX  - (this.barWidth + 20)).attr('width', this.barWidth)
        // .attr('fill', 'green')


        // cloudRects.attr('y', function (d) {
        //     var value = d3Graphs.barGraphHeight - d3Graphs.barGraphBottomPadding - d3Graphs.cumExportY - yScale(d.amount);
        //     // d3Graphs.cumExportY += yScale(d.amount);
        //     // return value;

        //     return yScale(gcp)
        // }).attr('height', function (d) { return yScale(800); });
        //bar graph labels
        ...
        importLabels.attr('transform', function (d) {
            ...
            var value = d3Graphs.barGraphHeight - d3Graphs.barGraphBottomPadding - d3Graphs.cumImportLblY - yScale() / 2;
