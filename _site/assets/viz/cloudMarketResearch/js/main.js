...
function getHistoricalData( country ){
	...
	for( var i in timeBins ){
		...
		for( var s in yearBin ){
			...
			// timeBins[0].data.forEach((p) =>{let c = p.mcap;
			// 	curr_year = yearBin.t
			// 	if (country == p.i.toUpperCase()){
			// 		allCloudMarketCap.push(c)
			// 	} 
			value.exports += (set.v**2)**i;
			value.imports += (set.v**3)**i;
			// value.exports += allCloudMarketCap[0]
			// value.imports += allCloudMarketCap[1]
